...
/// Per-interval live callback for embedding consumers (local LinkGauge patch):
/// invoked by the reporter for every emitted interval (periodic ticks, the
/// final partial flush, and omitted warm-up rows), so a GUI can chart metrics
/// in real time instead of waiting for the run to end.
#[derive(Clone)]
pub struct IntervalHook(Arc<dyn Fn(&crate::json_report::Interval) + Send + Sync>);

impl IntervalHook {
    pub fn new(f: impl Fn(&crate::json_report::Interval) + Send + Sync + 'static) -> Self {
        Self(Arc::new(f))
    }

    fn invoke(&self, interval: &crate::json_report::Interval) {
        (self.0)(interval);
    }
}

// The closure field carries no Debug/PartialEq — hand-roll both so
// `Client`/`Server` keep their derived impls (same pattern as InterruptWatch).
impl std::fmt::Debug for IntervalHook {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.write_str("IntervalHook(..)")
    }
}

impl PartialEq for IntervalHook {
...
pub struct IntervalReporterConfig {
    ...
    /// Per-interval live callback (local LinkGauge patch); fired after each
    /// emitted interval, before any collecting/printing branch.
    pub on_interval: Option<IntervalHook>,
        ...
        let mut emit_interval = |start: f64,
            ...
            if do_emit {
                ...
                if collecting {
                    ...
                    // Live callback hook (local LinkGauge patch): fired before
                    // the collecting/printing branches so embedders see every
                    // interval regardless of output mode.
                    if let Some(hook) = config.on_interval.as_ref() {
                        hook.invoke(&interval);
                    }
