...
/// Live per-interval callback (local LinkGauge patch): attach via
/// [`ClientBuilder::on_interval`] / [`ServerBuilder::on_interval`] to receive
/// every emitted [`json_report::Interval`] in real time — periodic ticks, the
/// final partial flush, and omitted warm-up rows.
pub use reporter::IntervalHook;

/// The riperf3 library version (local LinkGauge patch): the vendored crate's
/// own `CARGO_PKG_VERSION`, for embedders to display/log.
