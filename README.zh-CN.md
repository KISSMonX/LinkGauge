...
- 应用内更新：启动后静默查询 GitHub Releases，有新版本时在「关于」按钮上标红点；下载安装包和重启安装都需用户明确点击，不会未经同意占用带宽。安装包经签名校验，支持 Windows NSIS、macOS `.app`、Linux AppImage（`deb` / `rpm` 安装的版本会引导到 Releases 页面手动下载）
...
- **houming** — 项目作者与维护者
- **DeepSeek** — AI 辅助开发伙伴（0.2.0 版本周期的 iperf3 选项集、服务端改进与问题修复）
