...
pub async fn generate_report(app: AppHandle, request: ReportRequest) -> Result<String, String> {
    ...
    match request.format.to_lowercase().as_str() {
        "html" => write_html(dir.join(format!("iperf3-report-{stamp}.html")), &request).await,
        "pdf" => write_pdf(dir.join(format!("iperf3-report-{stamp}.pdf")), &request).await,
...
async fn write_html(path: PathBuf, request: &ReportRequest) -> Result<String, String> {
    ...
    let logs = request
        ...
        .collect::<String>();
    let html = format!(
        r#"<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><title>iperf3 测试报告</title><style>body{{font:14px Arial,'Microsoft YaHei';max-width:1000px;margin:35px auto;color:#172033}}h1{{color:#096edc}}section{{border:1px solid #dce2ea;border-radius:8px;padding:18px;margin:16px 0}}pre{{background:#f6f8fb;padding:14px;white-space:pre-wrap}}table{{width:100%;border-collapse:collapse}}th,td{{padding:9px;border-bottom:1px solid #e5e9ef;text-align:right}}th:first-child,td:first-child{{text-align:left}}.logs{{font:12px Consolas;max-height:360px;overflow:auto}}</style></head><body><h1>iperf3 GUI 测试报告</h1><p>生成时间：{}</p><section><h2>测试配置</h2><pre>{}</pre></section><section><h2>统计结果</h2><pre>{}</pre></section><section><h2>测试数据</h2><table><thead><tr><th>时间(s)</th><th>带宽(Mbps)</th><th>传输(MB)</th><th>抖动(ms)</th><th>丢包率</th></tr></thead><tbody>{}</tbody></table></section><section><h2>执行日志</h2><div class="logs">{}</div></section></body></html>"#,
...
async fn write_pdf(path: PathBuf, request: &ReportRequest) -> Result<String, String> {
    ...
    let loss = request
        ...
        .unwrap_or(0.0);
    let lines = vec![
        "iperf3 GUI Test Report".to_string(),
