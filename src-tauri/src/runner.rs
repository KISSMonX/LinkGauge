...
#[cfg(test)]
mod tests {
    use super::{append_log, is_server_unreachable, TestLog};
    ...
    fn ip_version_rejects_invalid_values() {
        ...
        for valid in [0, 4, 6] {
            ...
            assert!(
                crate::validation::validate(&req).is_ok(),
                "ip_version={valid} 应通过"
            );
        }
        for invalid in [1, 3, 5, 7, 255] {
            req.ip_version = invalid;
            assert!(
                crate::validation::validate(&req).is_err(),
                "ip_version={invalid} 应被拒绝"
            );
        }
    }
