...
import type { BackendEvent, IperfRuntimeInfo, LogEntry, MetricPoint, NetworkInfo, TestConfig, TestItem, TestSummary } from './types'

const defaults: TestConfig = { mode: 'client', protocol: 'tcp', serverIp: '192.168.1.100', port: 5201, duration: 30, parallel: 4, bandwidth: 100, packetLength: 128, interval: 1, iperfPath: 'bundled' }
...
const local = ref<NetworkInfo>({ ip: '127.0.0.1', mac: '--', hostname: 'localhost', interfaceName: '默认网卡' })
...
function reset() { config.value = { ...defaults, mode: config.value.mode, protocol: config.value.protocol }; log('INFO', '参数已恢复默认值') }
...
async function runNext() {
  ...
  try {
    activeSession.value = await invoke<string>('start_test', { request: { taskId, ...config.value } })
  } catch (error) {
...
onMounted(async () => {
  ...
  if (isTauri()) { try { [local.value, runtime.value] = await Promise.all([invoke<NetworkInfo>('get_network_info'), invoke<IperfRuntimeInfo>('get_iperf_runtime_info')]); unlisten = await listen<BackendEvent>('test-event', (e) => handleEvent(e.payload)); log(runtime.value.available ? 'INFO' : 'WARN', runtime.value.available ? `${runtime.value.version} 已就绪（${runtime.value.bundled ? '内置' : '系统'}）` : '未找到可用的 iperf3 运行时') } catch (e) { log('WARN', `系统信息读取失败：${e}`) } }
...
<template>
  <div class="app-shell">
    <div class="titlebar"><div class="brand-icon">⌁</div><h1>iperf3 GUI 测试工具</h1></div>
    ...
    <div class="workspace"><ConfigPanel :config="config" :items="items" :running="running" :recovery="!!recovery" :runtime="runtime" @update:config="config = $event" @toggle-item="toggleItem" @reset="reset" @start="start" @stop="stop" @report="generateReport('html')" @clear="clearLogs" /><Dashboard :local="local" :config="config" :current="current" :points="points" :progress="progress" :elapsed="elapsed" :summary="summary" :connected="connected" /><StatusPanel :items="items" :logs="logs" :progress="progress" :elapsed="elapsed" :duration="config.duration" @clear="clearLogs" /></div>
