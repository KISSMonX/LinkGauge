...
import type { NetworkInfo, TestConfig, TestItem, MetricPoint, TestSummary } from '../types'
import BandwidthChart from './BandwidthChart.vue'

const props = defineProps<{ local: NetworkInfo; config: TestConfig; current?: TestItem; points: MetricPoint[]; progress: number; elapsed: number; summary: TestSummary; connected: boolean }>()
...
<template>
  <main class="panel dashboard">
    ...
    <div class="current-row">
      <section class="current-test"><h3>当前执行项</h3><strong>{{ current ? `${current.label}` : '等待开始测试' }}</strong><p>{{ current ? (current.protocol === 'ping' ? '正在检测网络连通性' : `正在测试（客户端 → 服务端）`) : '请选择测试项目并开始' }}</p></section>
