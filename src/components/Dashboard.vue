...
<template>
  <main class="panel dashboard">
    ...
    <div class="network-grid">
      <section><h3>{{ t('dash.local') }}</h3><dl><dt>{{ t('dash.ip') }}</dt><dd>{{ local.ip }}</dd><dt>{{ t('dash.nic') }}</dt><dd>{{ local.interfaceName }}</dd><dt>{{ t('dash.nicSpeed') }}</dt><dd>{{ local.speedMbps ? local.speedMbps + ' Mbps' : t('dash.unknown') }}</dd><dt>{{ t('dash.mac') }}</dt><dd>{{ local.mac }}</dd><dt>{{ t('dash.hostname') }}</dt><dd>{{ local.hostname }}</dd></dl></section>
      ...
      <section class="connection"><div class="section-title"><h3>{{ t('dash.conn') }}</h3><span :class="['status-pill', connected ? 'ok' : 'idle']">{{ connected ? t('dash.connected') : t('dash.disconnected') }}</span></div><dl><dt>{{ t('dash.rtt') }}</dt><dd>{{ summary.pingAverage ? summary.pingAverage.toFixed(2) : '--' }} ms</dd><dt>{{ t('dash.loss') }}</dt><dd>{{ summary.lossPercent.toFixed(2) }}%</dd><dt>Local Port</dt><dd>{{ localPort || '--' }}</dd><dt>{{ t('dash.connTime') }}</dt><dd>{{ formatTime(elapsed) }}</dd></dl></section>
    </div>
    ...
    <section class="chart-card">
      <h3>{{ chartTitle }}</h3>
      <div class="chart-content"><div class="chart"><BandwidthChart :points="points" :live="live" /></div><dl class="metrics"><dt>{{ t('dash.currentBw') }}</dt><dd class="blue">{{ currentBandwidth.toFixed(2) }} Mbps</dd><dt>{{ t('dash.avgBw') }}</dt><dd>{{ summary.averageBandwidth.toFixed(2) }} Mbps</dd><dt>{{ t('dash.minBw') }}</dt><dd>{{ summary.minBandwidth.toFixed(2) }} Mbps</dd><dt>{{ t('dash.maxBw') }}</dt><dd>{{ summary.maxBandwidth.toFixed(2) }} Mbps</dd><dt>{{ t('dash.totalTransfer') }}</dt><dd>{{ summary.totalTransferMb.toFixed(2) }} MB</dd><dt>{{ t('dash.retrans') }}</dt><dd>0.00%</dd></dl></div>
    </section>
  </main>
</template>
